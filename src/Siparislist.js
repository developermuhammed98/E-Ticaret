...
function Siparislist() {
  const [siparislertablo, setSiparislertablo] = useState([]);
  useEffect(() => {
    const fetchSiparislertablo = async () => {
      try {
        const response = await axios.get(
          "https://private-f738ac-itoapi3.apiary-mock.com/siparislertablo"
        );
        setSiparislertablo(response.data.Siparislerlist); // JSON verisindeki "Cargos" dizisini state'e kaydediyoruz
      } catch (error) {
        console.error("Kargo verileri alınırken hata oluştu:", error);
      }
    };

    fetchSiparislertablo();
  }, []);
  return (
    <div>
                ...
                <div className="portlet light">
                  ...
                  <div className="portlet-body flip-scroll">
                    <table className="table table-bordered table-striped table-condensed flip-content">
                      ...
                      <tbody>
                        {siparislertablo.map((item, index) => (
                          <tr key={index}>
